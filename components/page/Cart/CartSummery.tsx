...
function CartSummery({
  ...
  return (
    <div className=" w-full flex-1 flex flex-col  border border-gray-50 rounded-2xl justify-between h-fit  py-6 px-5 lg:sticky lg:top-28 ">
          ...
          >
            {Data?.map((item: sansCartItems, index: number) => {
              return (
                <div
                  key={index}
                  className="w-full rounded-2xl bg-gray-50 px-4 py-5"
                >
      ...
      {CardPayment &&
      (totalDiscount !== 0 ? totalPrice - totalDiscount : totalPrice) >=
      ...
      <div
        className={`flex flex-col gap-4 pt-4 ${
          totalDiscount === 0 ? "" : "border-t border-dashed"
        } border-gray-50`}
      >
            ...
            <NumericFormat
              value={
                ...
                (totalDiscount !== 0
                  ...
                  : totalPrice) >= balance?.data?.value.balance
                  ? (totalDiscount !== 0
                      ? totalPrice - totalDiscount
                      : totalPrice) - balance?.data?.value.balance
                  : totalDiscount !== 0
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {step == 2 ? (
            <button
              onClick={back}
              className="px-6  h-[48px] lg:h-[56px] border disabled:cursor-wait disabled:opacity-25 border-gray-100 rounded-2xl text-gray-400 flex items-center gap-2 text-sm font-semibold hover:bg-error-600 hover:text-white group hover:border-transparent duration-150"
            >
              انصراف
            </button>
          ) : null}
