...
function CartProductCards({ data }: { data: CartProductCardsType }) {
    ...
    <>
      <div className="w-full border border-gray-50 p-7 rounded-2xl hover:border-gray-100 hover:shadow-CMSHADOW duration-200 relative ">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-5 pb-7 border-b border-dashed border-gray-50">
          <Link
            href={`/service/${persianToSlug(data.serviceName)}-${persianToSlug(
              data.sportComplex.name
            )}-${data.service.id}`}
          >
            ...
            <div className="flex flex-col gap-4">
              <Link
                href={`service/${persianToSlug(
                  data.serviceName
                )}-${persianToSlug(data.sportComplex.name)}-${data.service.id}`}
              >
                <h2>
            ...
            <div className="w-full flex items-end justify-between">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() =>
                      UpdateCartCountHandler.mutate({
                        id: data.id,
                        count: data.count + 1,
                      })
                    }
                    disabled={UpdateCartCountHandler.isPending}
                    className={`CartAddButtons `}
                  >
                    +
                  </button>
                  <p>{data.count}</p>
