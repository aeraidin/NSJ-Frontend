...
function CartProductCards({ data }: { data: CartItems }) {
    ...
    return (
        <>
            <Toast messege={DeleteCartHandler.error ? (DeleteCartHandler.error as unknown as string) : "با موفقیت حذف شد"} Close={() => setResult(false)} isError={DeleteCartHandler.isError} isSuccess={DeleteCartHandler.isSuccess} Result={Result} />
            <div className='w-full border border-gray-50 p-7 rounded-2xl hover:border-gray-100 hover:shadow-CMSHADOW duration-200 relative '>
                <button onClick={() => DeleteCartHandler.mutate(data.id)} className='border border-error-500/30 p-2 rounded-xl bg-white hover:bg-error-600 hover:text-white text-error-600 absolute left-3 top-5 duration-200 z-20'>
