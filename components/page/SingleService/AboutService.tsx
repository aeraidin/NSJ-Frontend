...
function AboutService({ param }: AboutServiceProps) {
  const [expanded, setExpanded] = useState(false);
  // const about = useGetSingleService({ id: param });
  ...
  return (
    <div className="flex gap-y-6 flex-col py-10">
      {/* <h2>{`معرفی ${about.data?.value?.service.name}`}</h2> */}
      <div className="w-full  overflow-hidden">
        {/* <Collapse isOpened={expanded}>
          <p></p>
        </Collapse> */}

        <p className=" ">{expanded ? para : `${para.substring(0, 550)}... `}</p>
