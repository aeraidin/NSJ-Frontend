...
function Page() {
  ...
  //   const Data = [dat?.packagelist + dat?.sanslist];
  ...
  const Das = data?.data?.value?.sansList;
  const Dap = data?.data?.value?.packageList;
  const alphaNumeric = Das?.concat(Dap);
  const final = {
    alphaNumeric,
    totalPrice: data?.data?.value?.totalPrice,
    totalDiscount: data?.data?.value?.totalDiscount,
  };
  const RenderComponent = () => {
    switch (step) {
      case 1:
        return <Cart Data={alphaNumeric} />;
      case 2:
        return (
          <Payment ChangeState={(e) => setCardPayment(e)} state={CardPayment} />
        );
      default:
        return <Cart Data={alphaNumeric} />;
    }
  ...
  return (
    <MainLayout>
      ...
      ) : final?.alphaNumeric?.length > 0 || final?.alphaNumeric?.length > 0 ? (
        <div className="w-full flex-col lg:flex-row flex  gap-6 py-6">
          ...
          <div className="w-full flex flex-col gap-6 flex-1">
            ...
            <div className="w-full lg:hidden ">
              <CartSummery
                ...
                Data={step === 2 ? final.alphaNumeric : null}
                onClick={CheckOutHandler}
                disabled={PaymentMutation.isPending || !alphaNumeric}
          ...
          <div className="hidden lg:block w-full max-w-[430px]">
            <CartSummery
              ...
              Data={step === 2 ? final.alphaNumeric : null}
              onClick={CheckOutHandler}
              disabled={PaymentMutation.isPending || !Data}
              totalDiscount={final?.totalDiscount}
              totalPrice={final?.totalPrice}
            />
          </div>
        ...
        <div className="w-full flex-col lg:flex-row flex  gap-6 py-6">
          <div className="flex flex-1 items-center justify-center flex-col gap-4 border border-gray-50  max-w-[500px] w-full mx-auto py-10 rounded-2xl">
            ...
            <Link href={"/"} className="w-full max-w-[250px]">
              <PrimaryBtn>ادامه خرید </PrimaryBtn>
            </Link>
          </div>
        </div>
      )}
